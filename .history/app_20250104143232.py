    ...
    if current_user.is_admin:
        try:
            ...
            if admin_data:
                ...
                db.session.commit()
                return jsonify({'status': 'success', 'message': 'Admin credentials updated successfully'})
            else:
...
def general():
    ...
    for attendance in attendances:
        ...
        attendance_data.append({
            ...
            "status": "Completed" if attendance.check_out else "In Progress",
        })
...
def handle_attendance(employee):
    ...
    if attendance:
        if not attendance.check_out:
            ...
            return {
                'status': 'success',
                'message': f'Check-out successful at {now.strftime("%I:%M %p")}',
                'type': 'check_out'
            }
        else:
            return {
        ...
        return {
            'status': 'success',
            'message': f'Check-in successful at {now.strftime("%I:%M %p")}',
            'type': 'check_in'
        }
...
def facial_recognition():
    ...
    for i in os.listdir("static/uploads/profiles/"):
        if emp_id in i:
            ...
            break
    if not profile_pic:
        return jsonify({'status': 'error', 'message': 'Please update profile picture'}), 404
    ...
    if len(biden_encoding) == 0 or len(unknown_encoding) == 0:
        return jsonify({'status': 'error', 'message': 'No face detected in one or both images'}), 404
    ...
    if results[0]:
        ...
        return jsonify({'status': 'error', 'message': 'The faces do not match. Please ensure the face is clearly visible.'})
...
# Change password form submission endpoint
@login_required
def change_password():
